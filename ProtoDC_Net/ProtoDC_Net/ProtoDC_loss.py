...
class ProtoDC_loss(nn.Module):
    def __init__(self):
        super(ProtoDC_loss, self).__init__()
    ...
    def forward(self, views, target):
        ...
        loss_gl = -(label_matrix * log_prob_i).sum(1) / (label_matrix.sum(1) + 1e-9)

        # local → global (column-wise softmax)
        log_prob_t = F.log_softmax(logits.T, dim=1)
        loss_lg = -(label_matrix * log_prob_t).sum(1) / (label_matrix.sum(1) + 1e-9)
        ...
        loss = (loss_gl.mean() + loss_lg.mean() + loss_gg.mean()) / 3
